...
Ink is just the narrative engine—it doesn't handle how the story looks, how saves work, or anything visual. It's designed to plug into something else: a game engine, a web template, whatever. That's where this web template comes in.

### Do I need to know how to code?

No. If you can write an email, you can write an Ink story. Ink uses plain text with a few simple symbols:

- `*` or `+` for choices
- `->` for "go to this part"
- `===` to name a section

That's most of it. The template handles all the web stuff through Ink tags—add images with `# IMAGE: photo.jpg`, play sounds with `# AUDIO: door.mp3`, and so on. No HTML, CSS, or JavaScript required.
...
No. This is an independent project. [Inkle](https://www.inklestudios.com/) created the Ink language and the [inkjs](https://github.com/y-lohse/inkjs) runtime that makes browser playback possible, but this template's interface, save system, and features are separate work.
